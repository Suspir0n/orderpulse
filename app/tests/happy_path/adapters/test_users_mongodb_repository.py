...
def test_update_user(mongo_client):
    ...
    result_updating = repository.update_user(USERS_DATA[1]['_id'], _user_data)
...
def test_delete_user(mongo_client):
    ...
    created_user_result = repository.create_user_one(USER_DATA)
    delete_user_result = repository.delete_user(_id=USER_DATA['_id'])
    has_user_with_id = repository.get_user_by_id(_id=USER_DATA['_id'])
